...
unsigned int nStakeMaxAge = 60.0 * 60.0 * 24.0 * 9.0;   // 9 days       
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "HoChiMinh Starts";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1500659800;
        }
        else
        {
            txNew.nTime    = 1500659810; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015
        }
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1500659820;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1812632;
        if (fTestNet)
        {
            block.nTime    = 0;
            block.nNonce   = 0;
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0x5b7924939fa99d0707ca3159f7bf7ab7a8594d3df5123730ab986e9a1d1fe73a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xc0, 0xb8, 0xd5 };
